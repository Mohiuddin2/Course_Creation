...
exports.getCourse = asyncHandler(async (req, res, next) => {
  ...
  if (!course) {
    return next(
      new ErrorRespose(`No Course with the id of ${req.params.id}`),
      404
    );
  }
  res.status(200).json({
...
exports.addCourse = asyncHandler(async (req, res, next) => {
  ...
  if (!bootcamp) {
    return next(
      new ErrorRespose(`No bootcamp with the id of ${req.params.bootcampId}`),
      404
    );
  }
...
if(bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin'){
  return next(
    new ErrorRespose(
      `User ${req.user.id} is not authorized to delete a course to bootcamp ${bootcamp._id}`, 401
    )
  );
}
